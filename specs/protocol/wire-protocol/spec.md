...
For v1 the protocol covers the scalar and set/EMITS execution paths and their side cases: `MT_CLIENT`, `MT_INFO`, `MT_META`, `MT_RUN`, `MT_NEXT`, `MT_EMIT`, `MT_DONE`, `MT_CLEANUP`, `MT_FINISHED`, `MT_CLOSE`, `MT_PING_PONG`, `MT_TRY_AGAIN`, `MT_RESET`. Single-call (`SC_FN_*`), virtual-schema adapter calls, and connect-back credential messages are out of scope for v1. The error close path uses the prefix `F-UDF-CL-RUST-####`.

## Scenarios

### Scenario: DEALER transport connects to the IPC socket

* *GIVEN* a valid `ipc://<socket_path>` address
* *WHEN* `ZmqTransport::connect` is called with that path
* *THEN* it MUST open a ZMQ DEALER socket connected to the address
...
* *GIVEN* a connected `ZmqTransport` paired with a fake ROUTER peer
* *WHEN* the client sends an `ExascriptRequest` and the peer replies with one `ExascriptResponse` frame
* *THEN* `send` MUST serialize the request to a single ZMQ frame using prost encoding
* *AND* `recv` MUST decode exactly one frame into an `ExascriptResponse`
...
* *GIVEN* a `Protocol` in its initial handshake phase
* *WHEN* a response arrives whose `message_type` is not valid for that phase (e.g. `MT_EMIT` before `MT_RUN`)
* *THEN* `on_response` MUST return a `ProtocolError` identifying the unexpected message type
* *AND* it MUST NOT panic or silently ignore the message
