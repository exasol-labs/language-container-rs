...
* *THEN* the crate MUST compile to a cdylib exporting the named entry point with the RETURNS output-shape marker
...
* *AND* the timezone resolution MUST be proven by the live-DB integration end-to-end scenario, not by a unit test, because zoneinfo resolution in the compiled cdylib depends on the container's bundled `tzdata` and cannot be exercised in a host unit test
