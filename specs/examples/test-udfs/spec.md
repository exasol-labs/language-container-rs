...
* *THEN* the crate MUST compile to a cdylib with `serde_json` statically linked and the RETURNS output-shape marker
...
* *WHEN* the example is built as a cdylib
...
* *WHEN* the crate is built as a cdylib
...
* *THEN* the crate MUST compile to a cdylib exporting the named entry point derived from the function identifier
...
* *THEN* the crate MUST compile to a cdylib exporting `__exa_udf_entry_SET_SUM` with the RETURNS output-shape marker
...
### Scenario: column-meta builds its output row from the call-site EMITS list

* *GIVEN* the `column-meta` crate with the `#[exasol_udf]` functions `describe_output`, `input_column_name`, and `emit_type_mismatch`
* *WHEN* `describe_output` builds its row from `output_column_count()` and `output_column(idx)` alone, and the same entry point is registered against one `EMITS` list and then another
...
* *AND* `input_column_name` MUST return the declared name and type of its input column
