...
* *WHEN* `describe_output` builds its row from `output_column_count()` and `output_column(idx)` alone and the same entry point is re-registered against a different `EMITS` list
