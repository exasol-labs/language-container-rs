...
The distributed SLC artifact is `lc-rs.tar.gz`, built by `Dockerfile.alpine` (`--target artifact`) and uploaded to BucketFS by `scripts/install.sh`. The tarball flattens the entire `alpine:3` rootfs plus the glibc/libstdc++ runtime libraries copied via `cp -L` from the `rust:1.92-bookworm` (Debian 12) builder. The cargo-generated `THIRD-PARTY-LICENSES.md` covers only the Rust dependency graph of `exaudfclient`, so the bundled OS/runtime third-party components carry attribution and source-offer obligations that an additional notice bundle must document. This is an attribution + source-offer compliance concern only; all components are unmodified upstream binaries shipped as separate files (glibc is LGPL-2.1; libstdc++/libgcc carry the GCC Runtime Library Exception), so nothing propagates to `exaudfclient` or to user UDFs.

The OS-layer notice bundle is **generated**, not hand-maintained: the boilerplate lives in `dist/` and a generator renders the license texts from `cargo about`'s embedded SPDX data, so no verbatim license text is committed to the repository.
...
* *GIVEN* the distributed SLC tarball (`lc-rs.tar.gz`, produced from `Dockerfile.alpine`) bundles `alpine:3` apk packages and the glibc/GCC runtime libs copied from the `rust:1.92-bookworm` builder, none of which are covered by the cargo-generated `THIRD-PARTY-LICENSES.md`
