...
* *AND* `emit` MUST take ownership of the row (`Vec<Value>`) and move each `Value` into the host's emit buffer
* *AND* `emit` MUST enforce the RETURNS ban: a call in `output_iter = ExactlyOnce` context MUST return `Err(UdfError)`
