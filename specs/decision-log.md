...
## ADR-057: ZmqTransport retries EAGAIN on recv/send up to a 120 s backstop
...
## ADR-058: Resolve issue #29 by spike-and-measure, not by argument
...
## ADR-059: Extend the benchmark (NUMERIC/DATE/TIMESTAMP shapes) before spiking
...
## ADR-060: Re-measuring the previously-dropped Arrow C Data Interface is warranted, and is a re-measurement — not a silent reversal
...
## ADR-061: The productionisation spec delta captures a behavioral invariant, not a chosen implementation
...
## ADR-062: Decision gate — promote Spike A (string-block fast-path), drop Spikes B and C
