...
A `.backend` of `local` covers two deployment shapes and only one of them has a transport this project owns. The shape that runs the database in a managed VM records an SSH endpoint and a node key, and the SSH/filesystem transport applies to it. The shape that runs the database in a container engine on the invoking host records neither, so `container/personal-install-local` reports the supported Personal CLI route instead of selecting a transport. The backend field alone therefore no longer decides the transport for a local deployment; the deployment directory's contents decide it.
...
* *AND* a `.backend` of `local` MUST select the SSH/filesystem transport described in `container/personal-install-local` only when the deployment provides both an SSH endpoint and a node key; a local deployment providing neither MUST be reported under that feature's no-transport scenario instead of entering a transport
