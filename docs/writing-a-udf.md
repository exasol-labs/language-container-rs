...
#   * Exasol Personal       — no HTTP endpoint; copy into its BucketFS directory
#                             (see the note below)
```
...
On **Exasol Personal** there is no BucketFS HTTP endpoint, so the upload step above does not apply. Copy the `.so` into the deployment's BucketFS directory by the same mechanism the SLC install uses — over SSH into the VM, or into the host directory the deployment shares into it — as described in [Deploying a UDF `.so` on Personal](installation.md#exasol-personal-install).
